...
export default function sustainable(
  ...
  const {
    cdnMappingsFile = './cdn-mappings.json',
    exclude = [],
    insertIntoHtml = true
  } = options
  ...
  const initializePlugin = async () => {
    ...
    try {
      ...
      const cdnMappingsPath = resolve(config.root || process.cwd(), cdnMappingsFile)
      if (existsSync(cdnMappingsPath)) {
        ...
        const defaultMappings: CDNMapping = {
          ...
          "class-variance-authority": "https://esm.sh/class-variance-authority@{version}"
        }
        
        // Write default mappings file
        await import('fs/promises').then(fs => 
          fs.writeFile(cdnMappingsPath, JSON.stringify(defaultMappings, null, 2))
        )
      ...
      const packageJsonPath = resolve(config.root || process.cwd(), 'package.json')
      ...
      const depsToProcess = Object.keys(cdnMappings).filter(dep => 
        allDependencies[dep] && !exclude.includes(dep)
      )

      // Read exact versions from lock file - only for deps we care about
      const exactVersions = await getExactVersions(config.root || process.cwd(), depsToProcess)
      
      // Create external dependencies map with exact versions for CDN-mapped dependencies only
      depsToProcess.forEach(dep => {
        const exactVersion = exactVersions[dep] || allDependencies[dep].replace(/^[\^~]/, '')
      ...
      console.log(`🌍 Sustainable Plugin: Externalizing ${Object.keys(importMap).length} dependencies`)
      console.log(`📋 External dependencies:`, Object.keys(importMap).join(', '))
      
      isInitialized = true
    } catch (error) {
      console.warn('❌ Sustainable Plugin: Failed to process dependencies:', error)
    }
  }
  ...
  return {
    ...
    async config(viteConfig) {
      ...
      if (external.length > 0) {
        ...
        if (Array.isArray(existingExternal)) {
          ...
          viteConfig.build.rollupOptions.external = (source: string, importer: string | undefined, isResolved: boolean) => {
    ...
    transformIndexHtml(html: string) {
      if (!insertIntoHtml || Object.keys(importMap).length === 0) {
        return html
      }
    ...
    <script type="importmap">
      ...
      if (moduleScriptIndex !== -1) {
        return html.slice(0, moduleScriptIndex) + importMapScript + '\n     ' + html.slice(moduleScriptIndex)
      } else {
...
async function getExactVersions(rootPath: string, filterDeps?: string[]): Promise<Record<string, string>> {
  ...
  try {
    ...
    if (existsSync(yarnLockPath)) {
      ...
      const yarnMatches = yarnLock.match(/^"?([^@\s]+)@[^"]*"?:\s*\n\s*version\s+"([^"]+)"/gm)
      if (yarnMatches) {
        yarnMatches.forEach(match => {
          const [, packageName, version] = match.match(/"?([^@\s]+)@[^"]*"?:\s*\n\s*version\s+"([^"]+)"/) || []
      ...
      if (packageLock.packages) {
        ...
        Object.keys(packageLock.packages).forEach(path => {
          if (path.startsWith('node_modules/')) {
            ...
            if (!packageName.includes('/')) { // Only top-level packages
